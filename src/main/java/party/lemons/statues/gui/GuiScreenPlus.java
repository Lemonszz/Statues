...
public class GuiScreenPlus extends GuiContainer {
	...
	protected void drawGuiContainerBackgroundLayer(float f, int bx, int by) {
		...
		Minecraft.getMinecraft().renderEngine.bindTexture(new ResourceLocation(backgroundTexture));
	...
	protected void drawGuiContainerForegroundLayer(int fx, int fy) {
		GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
		root.render();
		super.drawGuiContainerForegroundLayer(fx, fy);
	}
	...
	public void drawTiledRect(int rx, int ry, int rw, int rh, int u, int v, int tw, int th) {
		...
		for (int y = 0; y < rh; y += th) {
			for (int x = 0; x < rw; x += tw) {
				...
				bb.pos(x1, y2, this.zLevel).tex(u1, v2).endVertex();
				bb.pos(x2, y2, this.zLevel).tex( u2, v2).endVertex();
				bb.pos(x2, y1, this.zLevel).tex( u2, v1).endVertex();
				bb.pos(x1, y1, this.zLevel).tex( u1, v1).endVertex();
			}
		}
